...
import { Upload, Download, Move, ImagePlus } from 'lucide-react';
...
// Virtual try-on: client photo as background, design blended on top
// (multiply), draggable + scalable + rotatable, exportable to PNG.
export default function TryOnModal({ design, open, onOpenChange }) {
  ...
  const download = async () => {
    ...
    try {
      ...
      toast({ variant: 'destructive', description: t('designs.tryOn.downloadError') });
    } finally {
      setExporting(false);
    }
  };
      ...
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          <DialogTitle className="font-display text-xl">
            {t('designs.tryOn.title')} — {design.title}
          </DialogTitle>
          <DialogDescription>{t('designs.tryOn.description')}</DialogDescription>
        </DialogHeader>
        ...
        {!photo ? (
          ...
          >
            ...
            <span className="text-sm font-medium">{t('designs.tryOn.uploadPhoto')}</span>
            <span className="px-6 text-xs">{t('designs.tryOn.uploadHint')}</span>
          </button>
        ) : (
          <div className="space-y-4">
            <div
              ref={stageRef}
              className="relative h-[42vh] w-full select-none overflow-hidden rounded-lg bg-[#0c0f16]"
            >
              <img
                src={photo}
                alt=""
              ...
              <div className="pointer-events-none absolute bottom-2 left-1/2 flex -translate-x-1/2 items-center gap-1.5 whitespace-nowrap rounded-full bg-background/80 px-3 py-1 text-xs text-muted-foreground">
                ...
                {t('designs.tryOn.dragHint')}
              </div>
            </div>

            <div className="grid gap-4 sm:grid-cols-3">
              <div className="space-y-2">
                <Label className="text-xs text-muted-foreground">{t('designs.tryOn.scale')}</Label>
                <Slider
                  value={[scale]}
                  min={0.2}
                  max={2.5}
              ...
              <div className="space-y-2">
                <Label className="text-xs text-muted-foreground">{t('designs.tryOn.rotation')}</Label>
                <Slider
                  value={[rotation]}
                  min={-180}
                  max={180}
              ...
              <div className="space-y-2">
                <Label className="text-xs text-muted-foreground">{t('designs.tryOn.opacity')}</Label>
                <Slider
                  value={[opacity]}
                  min={20}
                  max={100}
            ...
            <div className="flex flex-wrap justify-end gap-2">
              <Button variant="outline" onClick={() => fileRef.current?.click()}>
                ...
                {t('designs.tryOn.changePhoto')}
              </Button>
              <Button onClick={download} disabled={exporting}>
                <Download className="mr-2 h-4 w-4" />
                {t('designs.tryOn.download')}
              </Button>
            </div>
          </div>
        )}
